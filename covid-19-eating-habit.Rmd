...
kable(head(cbind(file.restaurant %>% 
             select(-date_range_start_yymm, -date_range_end_yymm, -visitor_home_bg),
           file.restaurant %>% select(visitor_home_bg)), 5), format = "simple", row.names = F)
```
...
```{r correlation_btw_vars, fig.height = 7, fig.width = 10, dev.args = list(bg = 'transparent'), warning = FALSE, message=FALSE}
setwd(temp.wd)
load('nbhd_dataset.RData')
## correlation check
drop.row <- unique(which(is.na(dataset))%%nrow(dataset))

cont.vars <- dataset %>% select(ws, pop.den, emp.den, dist.to.grocery, r.num, f.ratio,
